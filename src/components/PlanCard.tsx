...
const PlanCard = ({ name, type, price, matchPercent, badge, logoColor, logoText }: PlanCardProps) => {
  ...
  return (
    <div className="glass-card rounded-xl p-4 w-[210px] relative animate-float border border-border/60" style={{ animationDelay: `${Math.random() * 2}s` }}>
      {/* Badge */}
      {badge && (
        <div className={`absolute -top-2.5 -right-2.5 px-2.5 py-0.5 rounded-full text-[10px] font-bold font-ui ${
          badge === "best"
      ...
      <div className="flex items-start gap-3 mb-3">
        <div
          className="w-11 h-11 rounded-lg flex items-center justify-center text-[8px] font-bold font-ui shrink-0"
      ...
      <div className="flex items-center justify-between gap-2">
        <button className="flex-1 border-2 border-primary bg-primary/20 hover:bg-primary/30 text-primary text-[10px] font-display font-bold px-3 py-2 rounded-md tracking-widest transition-all">
          VIEW WHY?
        </button>
        <div className={`w-14 h-14 rounded-full border-2 flex flex-col items-center justify-center bg-gradient-to-br ${matchBg} ${matchColor}`}
