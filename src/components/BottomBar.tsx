...
const BottomBar = () => {
  return (
    <div className="w-full px-6 py-3 glass-surface flex items-center justify-between">
      ...
      {/* Center CTA */}
      <button className="flex items-center gap-3 px-8 py-3 rounded-full font-display text-xs font-bold tracking-widest transition-all glow-primary"
        style={{
          background: 'linear-gradient(135deg, hsl(199 89% 48%), hsl(210 80% 45%))',
        }}>
      ...
      {/* Right */}
      <div>
        <Sparkles className="w-6 h-6 text-glow-gold animate-pulse" />
      </div>
    </div>
  );
};
